    ...
    getPromotionByID(id){
      this.promotionService.getPromotionById(id).subscribe(promo =>{
        ...
        if(this.promotion.listCakeIn[0] !="")
        {
          ...
          var stringListCakeID=""
          for(let index of this.promotion.listCakeIn)
          {
            stringListCakeID+=index+","
          }
          console.log(stringListCakeID.length)
          this.promotion.listCakeIn=   stringListCakeID.substring(0,stringListCakeID.length-1)
